...
void received_disconnect(int sender_id){
    ...
    printf("Clients %d and %d disconnected succesfully", sender_id, idx_interlocutor);
}
...
void received_connect(msg *message){
    ...
    if(clients[idx_interlocutor].interlocutor >= 0){
    ...
    printf("Succesfully connected %d and %d", id_sender, id_interlocutor);
}
...
int main(){
    ...
    msg received;
    while(1){
        if(msgrcv(server_queue_id, &received, max_msg_size, -7, 0) == -1){
